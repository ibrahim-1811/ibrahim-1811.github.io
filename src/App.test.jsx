...
it('renders the five flagship projects in approved order', () => {
  ...
  expect(within(work).getAllByRole('article').map(article => article.id)).toEqual([
    'simplr','mrta','invite-industrial-manipulation','intrinsic-ai-challenge','robothon-2025',
  ]);
...
it('renders research, experience, capabilities, about and contact', () => {
  renderApp();
  expect(screen.getByRole('region',{name:'Research and Publications'})).toHaveTextContent('PlanRob, ICAPS 2026');
  expect(screen.getByRole('region',{name:'Experience'})).toHaveTextContent('INVITE GmbH');
  expect(screen.getByRole('region',{name:'Technical Capabilities'})).toHaveTextContent('Isaac Mimic');
  expect(screen.getByRole('region',{name:'About'})).toHaveTextContent('June 2027');
  expect(within(screen.getByRole('contentinfo')).getByRole('link',{name:/email/i})).toHaveAttribute('href','mailto:immemon1811@gmail.com');
});
...
it('closes mobile navigation after selecting a destination', () => {
  ...
  fireEvent.click(within(screen.getByRole('navigation',{name:'Mobile'})).getByRole('link',{name:'Research'}));
