...
export default function ProjectFeature({ project, compact = false }) {
  return (
    <article
      id={project.slug}
      className={`project-feature${compact ? ' project-feature--compact' : ''}`}
      ...
      <div className="project-copy">
        ...
        {project.caseStudyHref && (
          <a className="text-link" href={project.caseStudyHref}>
            View Case Study <Icon name="external" size={18} />
          </a>
        )}
      </div>
      {(project.system || project.media.length > 0) && (
        <div className="project-visual">
          {project.system ? (
            <SystemSummary system={project.system} />
          ) : (
            <MediaFigure
              media={project.media[0]}
              className={project.slug === 'robothon-2025' ? 'media-figure--portrait' : ''}
            />
          )}
        </div>
      )}
